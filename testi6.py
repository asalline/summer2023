...
space = odl.uniform_discr([-1, -1], [1, 1], [100,100], dtype='float32')
...
for k in range(amount_of_data):
    ...
    ellipsoids = ([1, np.random.rand(), np.random.rand(), np.random.rand()*2 - 1, np.random.rand()*2 -1, 0.] for i in range(3))
    image = odl.phantom.geometric.ellipsoid_phantom(space, ellipsoids)
    image.asarray()[:] /= np.max(image.asarray())
    x.append(image)


    gaussian_noise = np.random.normal(0, 0.1, size=(100,100))
...
LPDnet = Architecture(forward_operator, adjoint_operator, Primal_nets, Dual_nets, (5, length, length), (5, length, length), iterations)
