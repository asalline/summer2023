...
import numpy as np
from GPU_test_functions import get_images, geometry_and_ray_trafo, UNet
...
test_images = get_images('/scratch2/antti/summer2023/test_walnut', amount_of_images=200, scale_number=4)
...
indices = np.random.permutation(test_rec_images.shape[0])[:30]
...
def train_network(net, loss_function, n_train=300, batch_size=25): #g_train, g_test, f_train, f_test, 

    ### Defining optimizer, ADAM is used here
    optimizer = optim.Adam(UNet_parameters, lr=0.0005) #betas = (0.9, 0.99)
...
running_loss, running_test_loss, ready_to_eval = train_network(UNet, loss_function, n_train=5000, batch_size=2) # g_train, g_test, f_train, f_test
