...
LGD = LGD(adjoint_operator_module, ray_transform_module, regularizer, f_images, \
          g_sinograms, f_rec_images, in_channels=2, out_channels=1, step_length=0.005, n_iter=5).to(device)
...
def train_network(net, f_images, g_sinograms, f_rec_images, f_test_rec_images, \
    ...
    for i in range(n_train):
        ...
        optimizer.zero_grad()
        print('shapeeee f', f_batch2.shape)
        print('shapeeee sinos', g_batch.shape)
...
running_loss, running_test_loss, ready_to_eval = train_network(LGD, f_images, \
...
# torch.save(ready_to_eval.state_dict(), '/scratch2/antti/networks/'+'LGS1_005.pth')
