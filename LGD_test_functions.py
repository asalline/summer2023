...
class LGD(nn.Module):
    def __init__(self, adjoint_operator_module, operator_module, regularizer, \
                 f_images, g_sinograms, f_rec_images, in_channels, out_channels, memory, n_iter):
        super().__init__()
        ...
        self.conv1 = nn.Conv2d(in_channels=self.in_channels+self.memory, \
        ...
        self.relu4 = nn.ReLU()
        self.conv5 = nn.Conv2d(in_channels=32, out_channels=self.out_channels+self.memory, \
    ...
    def forward(self, f_rec_images, g_sinograms):
        ...
        for i in range(self.n_iter):
            ...
            grad_f = self.gradient_of_f(f_sinogram - g_sinograms)
            # grad_reg = 0.1 * self.gradient_of_regularizer(f_rec_images)

            # print('grad-f', grad_f.shape)
            # print('grad-reg', grad_reg.shape)
            # print('f', f_rec_images.shape)
            # print('s', s.shape)
