...
class LGS(nn.Module):
    ...
    def forward(self, f_rec_images, g_sinograms):
        ...
        for i in range(self.n_iter):
            ...
            # dual layer
            print(type(f_sinogram))
            print(type(g_sinograms))
...
image = get_images('/scratch2/antti/summer2023/test_walnut/', amount_of_images=1, scale_number=2)
...
LGS = LGS(adjoint_operator_module, ray_transform_module, \
          sinogram, noisy_image, in_channels=2, out_channels=1, step_length=0.1, n_iter=5).to(device)
LGS.load_state_dict(torch.load('/scratch2/antti/networks/'+'LGS1_005.pth'))

LGS.eval()


reco, _ = LGS(noisy_image[:,:,:], sinogram[:,:,:])
